  ...
  await runTest("2. Fingerprinting test", async () =>
    withManagedClient(async (client) => {
      ...
      try {
        ...
        // CreepJS always contains "bot" as a label in its UI — check for actual block pages instead
        if (!body || /headless detected|automation detected|access denied|blocked/i.test(body)) {
