...
- **Section commands cap at `--count=10`** on free (HTTP 400 `"could not override limit"` above 10). The example above `--count=50` fails on free — always use `--count=10` and paginate with `--after-id` for full coverage.
