...
function fetchPersonForLayout(session, uuid, layout) {
  ...
  return apiFetch(session,
    `/v4/data/entities/people/${uuid}?card_ids=${cardIds}&field_ids=${fieldIds}&layout_mode=${layout}`);
}
function viewPerson(session, input, view = 'v3') {
  const uuid = resolvePermalink(session, input);
  if (view === 'both') {
    const v2 = fetchPersonForLayout(session, uuid, 'view_v2');
    const v3 = fetchPersonForLayout(session, uuid, 'view_v3');
    return {
      properties: { ...(v2.properties || {}), ...(v3.properties || {}) },
      cards: { ...(v2.cards || {}), ...(v3.cards || {}) },
    };
  }
  return fetchPersonForLayout(session, uuid, view === 'v2' ? 'view_v2' : 'view_v3');
}
...
if (command === 'auth') {
  ...
  if (!input) {
    console.error('Usage: node crunchbase-people.mjs view <permalink|uuid> [--view=v3|v2|both]');
    process.exit(1);
  }
  ...
  console.log(`Fetching person: ${input} (view=${view})...`);
  const data = viewPerson(session, input, view);
