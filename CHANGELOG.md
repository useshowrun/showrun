...
- [added] `showrun agent` CLI command — run AI agents headlessly from the terminal
  ...
  - Supports `--conversation <id>` to resume sessions, `--headful` for debugging, and `--verbose` for detailed logs
- [added] `cliproxy` LLM provider — alternative to Anthropic provider that prepends system prompts to user messages, useful for compatibility with CLI Proxy APIs that strip system parameters
- [added] Shared pack editing workflows — packs and conversations are no longer 1:1; multiple conversations can link to the same pack, and deleting a conversation keeps the linked pack intact
- [added] `sharp` dependency for image-based Turnstile detection and solving — enables robust CAPTCHA bypass even with shadow-DOM widgets
- [fixed] `playwright-js` execution is now wrapped in a try-catch to prevent dashboard/runner crashes on script errors
